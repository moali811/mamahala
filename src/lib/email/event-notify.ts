...
export function generatePulseThresholdEmail(event: SmartEvent, pulseCount: number): string {
  ...
  const content = `
    <div style="background:#FFFFFF;border-radius:12px;padding:24px;margin:0 0 16px;">
      ...
      <a href="${SITE_URL}/admin/events" style="display:inline-block;padding:12px 24px;background:#7A3B5E;color:#FFFFFF;text-decoration:none;border-radius:8px;font-size:14px;font-weight:600;">
        Open Events Manager
      </a>
    </div>`;
