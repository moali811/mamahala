...
import { questions, dimensions, tiers, dimensionInsights } from '@/data/communication-style-quiz';
import {
  generateSessionId,
  decodeResults,
...
function CommunicationStyleQuizInner() {
  ...
  useEffect(() => {
    ...
    if (encoded) {
      ...
      if (decoded && decoded.q === 'communication-style') {
    ...
    <div className="bg-[#FAF7F2] min-h-screen">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#E8C4C0] via-[#F0D5CA] to-[#FAF0EC]">
        ...
        <div className="container-main relative pt-24 pb-28 md:pt-28 md:pb-32 text-center">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            <Breadcrumb
              ...
              items={[
                ...
                { label: isRTL ? 'هل نتحدّثُ نفسَ اللغة؟' : 'Are We Speaking the Same Language?' },
              ]}
            />
          </motion.div>
          <motion.div
          ...
          >
            <MessageSquareHeart className="w-8 h-8 text-[#7A3B5E]" />
          </motion.div>
          <motion.h1
          ...
          >
            {isRTL ? (
              <>
                هل نتحدّثُ <span className="text-[#7A3B5E] italic">نفسَ</span> اللغة؟
              </>
            ) : (
              <>
                Are We Speaking the <span className="text-[#7A3B5E] italic">Same</span> Language?
              </>
            )}
          </motion.h1>
          <motion.p
          ...
          >
            {isRTL
              ? 'اكتشفْ نقاطَ قوّةِ التواصلِ في علاقتِك والمجالاتِ التي يمكنُ تحسينُها. 12 سؤالاً فقط \u2014 حوالي 5 دقائق.'
              : 'Discover your communication strengths and areas for growth in your relationship. Just 12 questions \u2014 about 5 minutes.'}
          </motion.p>
        </div>
                ...
                <ScrollReveal>
                  <div className="bg-white rounded-3xl p-10 border border-[#F3EFE8] shadow-sm">
                    ...
                    <p className="text-[#6B6580] leading-relaxed mb-4">
                      {isRTL
                        ? 'سنطرحُ عليك 12 سؤالاً حولَ تواصلِك في علاقتِك تغطّي أربعةَ أبعادٍ رئيسيّة: الاستماعُ الفعّال، والتعبيرُ العاطفيّ، والإصلاحُ والتعافي، والاستجابةُ للتواصل.'
                        : "We'll ask you 12 questions about your relationship communication across four key dimensions: Active Listening, Emotional Expression, Repair & Recovery, and Turning Toward."}
                    </p>
                    <p className="text-[#8E8E9F] text-sm leading-relaxed mb-8">
                      {isRTL
                        ? 'أجِبْ بصدقٍ بناءً على تجربتِك الفعليّة في علاقتِك. لا توجدُ إجاباتٌ صحيحةٌ أو خاطئة.'
                        : 'Answer honestly based on your actual experience in your relationship. There are no right or wrong answers.'}
                    </p>
                    <Button
                      size="lg"
                ...
                <ScrollReveal>
                  <div className="bg-white rounded-3xl p-10 lg:p-14 border border-[#F3EFE8] shadow-[0_20px_60px_rgba(0,0,0,0.06)] text-center">
                    <p className="text-sm font-medium text-[#8E8E9F] uppercase tracking-wider mb-6">
                      {isRTL ? 'نتيجةُ التواصل' : 'Communication Score'}
                    </p>
                    ...
                    <div className="mt-8 p-5 bg-[#FAF7F2] rounded-xl border border-[#F3EFE8] text-xs text-[#8E8E9F] leading-relaxed">
                      ...
                      <p>
                        {isRTL
                          ? 'مبنيٌّ على أبحاثٍ قائمةٍ على الأدلّةِ في مجالِ العلاقاتِ الزوجيّةِ والتواصلِ بينَ الأزواج. للحصولِ على تقييمٍ شاملٍ ومُخصَّص، نوصي بحجزِ جلسةٍ مع مستشارٍ مؤهَّل.'
                          : 'Informed by evidence-based research in couples communication and relationship science. For a comprehensive, personalized evaluation, we recommend booking a session with a qualified counselor.'}
                      </p>
                    </div>
                          ...
                          try {
                            await fetch('/api/quiz-share', {
                              ...
                              body: JSON.stringify({
                                name: 'Anonymous',
                                email: '',
                                quizSlug: 'communication-style',
                                quizName: isRTL ? 'هل نتحدّثُ نفسَ اللغة؟' : 'Are We Speaking the Same Language?',
                                sessionId,
                                score: totalScore,
                                maxScore: 60,
