...
const serviceNames: Record<string, { en: string; ar: string }> = {
  ...
  'managing-big-emotions': { en: 'Managing Big Emotions', ar: 'إدارةُ المشاعرِ الكبيرة' },
...
function EmotionalIntelligenceQuizInner() {
  ...
  useEffect(() => {
    ...
    if (encoded) {
      ...
      if (decoded && decoded.q === 'emotional-intelligence') {
    ...
    <div className="bg-[#FAF7F2] min-h-screen">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#E8C4C0] via-[#F0D5CA] to-[#FAF0EC]">
        ...
        <div className="container-main relative pt-24 pb-28 md:pt-28 md:pb-32 text-center">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            <Breadcrumb
              ...
              items={[
                ...
                { label: isRTL ? 'تقييمُ الذكاءِ العاطفيّ' : 'Emotional Intelligence Assessment' },
              ]}
            />
          </motion.div>
          <motion.div
          ...
          >
            <Heart className="w-8 h-8 text-[#7A3B5E]" />
          </motion.div>
          <motion.h1
          ...
          >
            {isRTL ? (
              <>
                تقييمُ <span className="text-[#7A3B5E] italic">الذكاء</span> العاطفيِّ للطفل
              </>
            ) : (
              <>
                Child <span className="text-[#7A3B5E] italic">Emotional</span> Intelligence
              </>
            )}
          </motion.h1>
          <motion.p
          ...
          >
            {isRTL
              ? 'اكتشفْ نقاطَ القوّةِ العاطفيّةَ لطفلِك والمجالاتِ التي يمكنُ رعايتُها. 12 سؤالاً فقط \u2014 حوالي 5 دقائق.'
              : 'Discover your child\u2019s emotional strengths and the areas that can be nurtured. Just 12 questions \u2014 about 5 minutes.'}
          </motion.p>
        </div>
                ...
                <ScrollReveal>
                  <div className="bg-white rounded-3xl p-10 border border-[#F3EFE8] shadow-sm">
                    ...
                    <p className="text-[#6B6580] leading-relaxed mb-4">
                      {isRTL
                        ? 'سنطرحُ عليك 12 سؤالاً حول طفلِك (الأعمار 5-17) تغطّي أربعةَ أبعادٍ رئيسيّة: الوعيُ العاطفيّ، والتنظيمُ العاطفيّ، والتعاطفُ والرحمة، والمهاراتُ الاجتماعيّة.'
                        : "We\u2019ll ask you 12 questions about your child (ages 5\u201317) across four key dimensions: Emotional Awareness, Emotional Regulation, Empathy & Compassion, and Social Skills."}
                    </p>
                    <p className="text-[#8E8E9F] text-sm leading-relaxed mb-8">
                      {isRTL
                        ? 'أجِبْ بناءً على ما تراه في طفلِك في الأيّامِ العاديّة. لا توجدُ إجاباتٌ صحيحةٌ أو خاطئة.'
                        : 'Answer based on what you observe in your child on typical days. There are no right or wrong answers.'}
                    </p>
                    <Button
                      size="lg"
                ...
                <ScrollReveal>
                  <div className="bg-white rounded-3xl p-10 lg:p-14 border border-[#F3EFE8] shadow-[0_20px_60px_rgba(0,0,0,0.06)] text-center">
                    ...
                    <p className="text-sm font-medium text-[#8E8E9F] uppercase tracking-wider mb-6">
                      {isRTL ? 'نتيجةُ الذكاءِ العاطفيّ' : 'Emotional Intelligence Score'}
                    </p>
                    ...
                    <div className="mt-8 p-5 bg-[#FAF7F2] rounded-xl border border-[#F3EFE8] text-xs text-[#8E8E9F] leading-relaxed">
                      ...
                      <p>
                        {isRTL
                          ? 'مبنيٌّ على أبحاثٍ قائمةٍ على الأدلّةِ في مجالِ التعلُّمِ الاجتماعيِّ العاطفيِّ وعلمِ النفسِ التنمويّ. للحصولِ على تقييمٍ شاملٍ ومُخصَّص، نوصي بحجزِ جلسةٍ مع مستشارٍ مؤهَّل.'
                          : 'Informed by evidence-based research in social-emotional learning (SEL) and developmental psychology. For a comprehensive, personalized evaluation, we recommend booking a session with a qualified counselor.'}
                      </p>
                    </div>
                          ...
                          try {
                            await fetch('/api/quiz-share', {
                              ...
                              body: JSON.stringify({
                                ...
                                quizName: isRTL ? 'تقييمُ الذكاءِ العاطفيِّ للطفل' : 'Child Emotional Intelligence Assessment',
                                sessionId,
                                score: totalScore,
                                maxScore: 60,
...
export default function EmotionalIntelligenceQuizPage() {
  return (
    <Suspense>
      <EmotionalIntelligenceQuizInner />
    </Suspense>
  );
}
