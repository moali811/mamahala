...
export default function ConflictStyleQuizPage() {
  ...
  const [selectedStyle, setSelectedStyle] = useState<ConflictStyle | null>(null);
  ...
  const handleAnswer = (questionId: string, style: ConflictStyle) => {
    ...
    <div className="bg-[#FAF7F2] min-h-screen">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#E8C4C0] via-[#F0D5CA] to-[#FAF0EC]">
        ...
        <div className="container-main relative pt-24 pb-28 md:pt-28 md:pb-32 text-center">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            <Breadcrumb
              ...
              items={[
                ...
                { label: isRTL ? 'كيفَ نتعاملُ مع الخلاف؟' : 'How Do We Fight?' },
              ]}
            />
          </motion.div>
          <motion.div
          ...
          >
            <Flame className="w-8 h-8 text-[#7A3B5E]" />
          </motion.div>
          <motion.h1
          ...
          >
            {isRTL ? (
              <>
                كيفَ <span className="text-[#7A3B5E] italic">نتعامل</span> مع الخلاف؟
              </>
            ) : (
              <>
                How Do We <span className="text-[#7A3B5E] italic">Fight</span>?
              </>
            )}
          </motion.h1>
          <motion.p
          ...
          >
            {isRTL
              ? 'اكتشفْ نمطَ تعاملِك السائدَ في الخلافات من خلالِ 16 موقفًا حقيقيًّا \u2014 حوالي 8 دقائق.'
              : 'Discover your dominant conflict style through 16 real-life scenarios \u2014 about 8 minutes.'}
          </motion.p>
        </div>
                ...
                <ScrollReveal>
                  <div className="bg-white rounded-3xl p-10 border border-[#F3EFE8] shadow-sm">
                    ...
                    <p className="text-[#6B6580] leading-relaxed mb-4">
                      {isRTL
                        ? 'سنعرضُ عليك 16 موقفًا من مواقفِ الخلافِ الحقيقيّة. في كلِّ موقف، اخترِ الاستجابةَ التي تصفُ أسلوبَك الأقرب. لا توجدُ إجاباتٌ صحيحةٌ أو خاطئة.'
                        : "We'll present you with 16 real conflict scenarios. For each one, choose the response that best describes your natural approach. There are no right or wrong answers."}
                    </p>
                ...
                <ScrollReveal>
                  <div className="bg-white rounded-3xl p-10 lg:p-14 border border-[#F3EFE8] shadow-[0_20px_60px_rgba(0,0,0,0.06)] text-center">
                    ...
                    <p className="text-sm font-medium text-[#8E8E9F] uppercase tracking-wider mb-6">
                      {isRTL ? 'نمطُ تعاملِك السائدُ في الخلاف' : 'Your Dominant Conflict Style'}
                    </p>
                    ...
                    <div className="mt-8 p-5 bg-[#FAF7F2] rounded-xl border border-[#F3EFE8] text-xs text-[#8E8E9F] leading-relaxed">
                      ...
                      <p>
                        {isRTL
                          ? 'مبنيٌّ على أبحاثٍ قائمةٍ على الأدلّةِ في مجالِ العلاقاتِ الزوجيّةِ وأبحاثِ الخلاف. للحصولِ على تقييمٍ شاملٍ ومُخصَّص، نوصي بحجزِ جلسةٍ مع مستشارٍ مؤهَّل.'
                          : 'Informed by evidence-based research in couples therapy and conflict resolution. For a comprehensive, personalized evaluation, we recommend booking a session with a qualified counselor.'}
                      </p>
                    </div>
                          ...
                          try {
                            await fetch('/api/quiz-share', {
                              ...
                              body: JSON.stringify({
                                name: 'Anonymous',
                                email: '',
                                quizSlug: 'conflict-style',
                                quizName: isRTL ? 'كيفَ نتعاملُ مع الخلاف؟' : 'How Do We Fight?',
                                sessionId,
