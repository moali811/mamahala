...
import { questions, dimensions, tiers, dimensionInsights } from '@/data/digital-life-quiz';
import {
  generateSessionId,
  decodeResults,
...
function DigitalLifeQuizInner() {
  ...
  useEffect(() => {
    ...
    if (encoded) {
      ...
      if (decoded && decoded.q === 'digital-life') {
    ...
    <div className="bg-[#FAF7F2] min-h-screen">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#E8C4C0] via-[#F0D5CA] to-[#FAF0EC]">
        ...
        <div className="container-main relative pt-24 pb-28 md:pt-28 md:pb-32 text-center">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            <Breadcrumb
              ...
              items={[
                ...
                { label: isRTL ? 'فحصُ حياتي الرقميّة' : 'My Digital Life Check-in' },
              ]}
            />
          </motion.div>
          <motion.div
          ...
          >
            <Smartphone className="w-8 h-8 text-[#7A3B5E]" />
          </motion.div>
          <motion.h1
          ...
          >
            {isRTL ? (
              <>
                فحصُ <span className="text-[#7A3B5E] italic">حياتي</span> الرقميّة
              </>
            ) : (
              <>
                My <span className="text-[#7A3B5E] italic">Digital Life</span> Check-in
              </>
            )}
          </motion.h1>
          <motion.p
          ...
          >
            {isRTL
              ? 'اكتشفْ كيفَ تؤثّرُ حياتُك الرقميّةُ على صحّتِك النفسيّة. 12 سؤالاً فقط \u2014 حوالي 5 دقائق.'
              : 'Discover how your digital life affects your mental well-being. Just 12 questions \u2014 about 5 minutes.'}
          </motion.p>
        </div>
                ...
                <ScrollReveal>
                  <div className="bg-white rounded-3xl p-10 border border-[#F3EFE8] shadow-sm">
                    ...
                    <p className="text-[#6B6580] leading-relaxed mb-4">
                      {isRTL
                        ? 'سنطرحُ عليك 12 سؤالاً حولَ علاقتِك بالتكنولوجيا ووسائلِ التواصلِ الاجتماعيّ، تغطّي أربعةَ أبعادٍ رئيسيّة: المقارنةُ الاجتماعيّة، والحدودُ الرقميّة، والمزاجُ وتقديرُ الذات، والنومُ والراحة.'
                        : "We'll ask you 12 questions about your relationship with technology and social media across four key dimensions: Social Comparison, Digital Boundaries, Mood & Self-Worth, and Sleep & Rest."}
                    </p>
                ...
                <ScrollReveal>
                  <div className="bg-white rounded-3xl p-10 lg:p-14 border border-[#F3EFE8] shadow-[0_20px_60px_rgba(0,0,0,0.06)] text-center">
                    <p className="text-sm font-medium text-[#8E8E9F] uppercase tracking-wider mb-6">
                      {isRTL ? 'نتيجةُ حياتِك الرقميّة' : 'Digital Life Score'}
                    </p>
                    ...
                    <div className="mt-8 p-5 bg-[#FAF7F2] rounded-xl border border-[#F3EFE8] text-xs text-[#8E8E9F] leading-relaxed">
                      ...
                      <p>
                        {isRTL
                          ? 'مبنيٌّ على أبحاثٍ قائمةٍ على الأدلّةِ في مجالِ الصحّةِ الرقميّةِ وعلمِ النفسِ التنمويّ. للحصولِ على تقييمٍ شاملٍ ومُخصَّص، نوصي بحجزِ جلسةٍ مع مستشارٍ مؤهَّل.'
                          : 'Informed by evidence-based research in digital wellness and developmental psychology. For a comprehensive, personalized evaluation, we recommend booking a session with a qualified counselor.'}
                      </p>
                    </div>

                    {/* Retake + Share with Counselor */}
                          ...
                          try {
                            await fetch('/api/quiz-share', {
                              ...
                              body: JSON.stringify({
                                name: 'Anonymous',
                                email: '',
                                quizSlug: 'digital-life',
                                quizName: isRTL ? 'فحصُ حياتي الرقميّة' : 'My Digital Life Check-in',
                                sessionId,
                                score: totalScore,
                                maxScore: 60,
...
export default function DigitalLifeQuizPage() {
  return (
    <Suspense>
      <DigitalLifeQuizInner />
    </Suspense>
  );
}
