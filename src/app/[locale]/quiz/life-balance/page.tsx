...
import { questions, dimensions, tiers, dimensionInsights } from '@/data/life-balance-quiz';
import {
  encodeResults,
  generateSessionId,
  decodeResults,
...
function LifeBalanceQuizInner() {
  ...
  useEffect(() => {
    ...
    if (encoded) {
      ...
      if (decoded && decoded.q === 'life-balance') {
  ...
  useEffect(() => {
    if (isResults && !isSharedView) {
      ...
      const encoded = encodeResults(payload);
    ...
    <div className="bg-[#FAF7F2] min-h-screen">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#E8C4C0] via-[#F0D5CA] to-[#FAF0EC]">
        ...
        <div className="container-main relative pt-24 pb-28 md:pt-28 md:pb-32 text-center">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            <Breadcrumb
              ...
              items={[
                ...
                { label: isRTL ? 'تقييمُ التوازنِ في الحياة' : 'Life Balance Assessment' },
              ]}
            />
          </motion.div>
          <motion.div
          ...
          >
            <Briefcase className="w-8 h-8 text-[#7A3B5E]" />
          </motion.div>
          <motion.h1
          ...
          >
            {isRTL ? (
              <>
                تقييمُ <span className="text-[#7A3B5E] italic">التوازن</span> والرضا في الحياة
              </>
            ) : (
              <>
                Life <span className="text-[#7A3B5E] italic">Balance</span> & Fulfillment
              </>
            )}
          </motion.h1>
          <motion.p
          ...
          >
            {isRTL
              ? 'اكتشف أين تزدهرُ وأين تحتاجُ المزيدَ من الاهتمام. 12 سؤالاً فقط \u2014 حوالي 5 دقائق.'
              : 'Discover where you are thriving and where you need more attention. Just 12 questions \u2014 about 5 minutes.'}
          </motion.p>
        </div>
                ...
                <ScrollReveal>
                  <div className="bg-white rounded-3xl p-10 border border-[#F3EFE8] shadow-sm">
                    ...
                    <p className="text-[#6B6580] leading-relaxed mb-4">
                      {isRTL
                        ? 'سنطرحُ عليك 12 سؤالاً تغطّي ستّةَ أبعادٍ رئيسيّةٍ للتوازنِ في حياتِك: المهنةُ والهدف، والعلاقات، والصحّةُ الجسديّة، والرّفاهيةُ العاطفيّة، والنموُّ الشخصيّ، والأمانُ الماليّ.'
                        : "We\u2019ll ask you 12 questions across six key dimensions of your life balance: Career & Purpose, Relationships, Physical Health, Emotional Wellbeing, Personal Growth, and Financial Security."}
                    </p>
                    <p className="text-[#8E8E9F] text-sm leading-relaxed mb-8">
                      {isRTL
                        ? 'لا توجدُ إجاباتٌ صحيحةٌ أو خاطئة \u2014 فقط كُنْ صادقًا مع نفسِك. نتائجُك خاصّةٌ تمامًا.'
                ...
                <ScrollReveal>
                  <div className="bg-white rounded-3xl p-10 lg:p-14 border border-[#F3EFE8] shadow-[0_20px_60px_rgba(0,0,0,0.06)] text-center">
                    ...
                    <p className="text-sm font-medium text-[#8E8E9F] uppercase tracking-wider mb-6">
                      {isRTL ? 'نتيجةُ التوازنِ في حياتِك' : 'Your Life Balance Score'}
                    </p>
                    ...
                    <div className="mt-8 p-5 bg-[#FAF7F2] rounded-xl border border-[#F3EFE8] text-xs text-[#8E8E9F] leading-relaxed">
                      ...
                      <p>
                        {isRTL
                          ? 'مبنيٌّ على أبحاثٍ قائمةٍ على الأدلّةِ في مجالِ أبحاثِ الرضا عن الحياةِ وأُطُرِ العافيةِ الشاملة. للحصولِ على تقييمٍ شاملٍ ومُخصَّص، نوصي بحجزِ جلسةٍ مع مستشارٍ مؤهَّل.'
                          : 'Informed by evidence-based research in life satisfaction research and holistic wellness frameworks. For a comprehensive, personalized evaluation, we recommend booking a session with a qualified counselor.'}
                      </p>
                    </div>
                          ...
                          try {
                            await fetch('/api/quiz-share', {
                              ...
                              body: JSON.stringify({
                                name: 'Anonymous',
                                email: '',
                                quizSlug: 'life-balance',
                                quizName: isRTL ? 'تقييمُ التوازنِ في الحياة' : 'Life Balance & Fulfillment Assessment',
                                sessionId,
                                score: totalScore,
                                maxScore: 60,
...
export default function LifeBalanceQuizPage() {
  return (
    <Suspense>
      <LifeBalanceQuizInner />
    </Suspense>
  );
}
