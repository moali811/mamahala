...
import { questions, dimensions, tiers, dimensionInsights } from '@/data/pre-marriage-quiz';
import {
  generateSessionId,
} from '@/lib/quiz-share';
...
const serviceNames: Record<string, { en: string; ar: string }> = {
  ...
  'couples-counseling': { en: 'Couples Counseling', ar: 'استشاراتُ الأزواج' },
};
...
export default function PreMarriageQuizPage() {
  ...
  const [step, setStep] = useState(0); // 0 = intro, 1-15 = questions, 16 = results
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#E8C4C0] via-[#F0D5CA] to-[#FAF0EC]">
        ...
        <div className="container-main relative pt-24 pb-28 md:pt-28 md:pb-32 text-center">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            <Breadcrumb
              ...
              items={[
                ...
                { label: isRTL ? 'فحصُ الجاهزيّةِ لِلزّواج' : 'Pre-Marriage Readiness Check' },
              ]}
            />
          </motion.div>
          <motion.div
          ...
          >
            {isRTL ? (
              <>
                فحصُ <span className="text-[#7A3B5E] italic">الجاهزيّة</span> لِلزّواج
              </>
            ) : (
              <>
                Pre-Marriage <span className="text-[#7A3B5E] italic">Readiness</span> Check
              </>
            )}
          </motion.h1>
          <motion.p
          ...
          >
            {isRTL
              ? 'اكتشفا مدى استعدادِكما لبناءِ حياةٍ معًا. 15 سؤالًا فقط \u2014 حوالي 7 دقائق.'
              : 'Discover how ready you are to build a life together. Just 15 questions \u2014 about 7 minutes.'}
          </motion.p>
        </div>
                ...
                <ScrollReveal>
                  <div className="bg-white rounded-3xl p-10 border border-[#F3EFE8] shadow-sm">
                    ...
                    <p className="text-[#6B6580] leading-relaxed mb-4">
                      {isRTL
                        ? 'سنطرحُ عليكما 15 سؤالًا تغطّي خمسةَ أبعادٍ رئيسيّةٍ للجاهزيّة: توافقُ القِيَم، وأساليبُ حلِّ الخلافات، ورؤيةُ الحياةِ والأسرة، والنهجُ الماليّ، وأنماطُ التواصل.'
                        : "We'll ask you 15 questions across five key dimensions of readiness: Values Alignment, Conflict Styles, Life & Family Vision, Financial Approach, and Communication Patterns."}
                    </p>
                    <p className="text-[#8E8E9F] text-sm leading-relaxed mb-8">
                      {isRTL
                        ? 'لا توجدُ إجاباتٌ صحيحةٌ أو خاطئة \u2014 أجيبا بصدقٍ كزوجَين. نتائجُكما خاصّةٌ تمامًا.'
                        : 'There are no right or wrong answers \u2014 answer honestly as a couple. Your results are completely private.'}
                    </p>
                    <Button
                      size="lg"
                ...
                <ScrollReveal>
                  <div className="bg-white rounded-3xl p-10 lg:p-14 border border-[#F3EFE8] shadow-[0_20px_60px_rgba(0,0,0,0.06)] text-center">
                    ...
                    <p className="text-sm font-medium text-[#8E8E9F] uppercase tracking-wider mb-6">
                      {isRTL ? 'نتيجةُ فحصِ الجاهزيّة' : 'Your Readiness Score'}
                    </p>
                    ...
                    <div className="mt-8 p-5 bg-[#FAF7F2] rounded-xl border border-[#F3EFE8] text-xs text-[#8E8E9F] leading-relaxed">
                      ...
                      <p>
                        {isRTL
                          ? 'مبنيٌّ على أبحاثٍ قائمةٍ على الأدلّةِ في مجالِ أبحاثِ الاستشاراتِ ما قبلَ الزواجِ ودراساتِ الجاهزيّة. للحصولِ على تقييمٍ شاملٍ ومُخصَّص، نوصي بحجزِ جلسةٍ مع مستشارٍ مؤهَّل.'
                          : 'Informed by evidence-based research in premarital counseling research and relationship readiness studies. For a comprehensive, personalized evaluation, we recommend booking a session with a qualified counselor.'}
                      </p>
                    </div>
                          ...
                          try {
                            await fetch('/api/quiz-share', {
                              ...
                              body: JSON.stringify({
                                name: 'Anonymous',
                                email: '',
                                quizSlug: 'pre-marriage',
                                quizName: isRTL ? 'فحصُ الجاهزيّةِ لِلزّواج' : 'Pre-Marriage Readiness Check',
                                sessionId,
                                score: totalScore,
                                maxScore: 75,
